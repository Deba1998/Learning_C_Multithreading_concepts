...
void parallel_merge_sort(int * array, unsigned int left, unsigned int right) {
	...
	if (left < right) {
		unsigned int mid = (left + right) / 2; // find the middle point
        boost::asio::post(pool, [array,left,mid](){parallel_merge_sort(array,left,mid);});
        boost::asio::post(pool, [array,mid,right](){parallel_merge_sort(array,mid+1,right);});
		// std::thread leftr (parallel_merge_sort,array, left, mid); 
		// std::thread rightr(parallel_merge_sort,array, mid+1, right);
		// // sequential_merge_sort(array, mid+1, right); // sort the right half
		// leftr.join();
		// rightr.join();
        pool.join();
